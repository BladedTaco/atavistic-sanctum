		...
		if (dead) {
			...
			for (var o = 0; o < min(array_length_2d(obj_results.kills, player_number), 4); o++) {
				...
				if (_inst >= 0) {
					with (other.player[_inst]) {
						...
						draw_sprite_ext(scr_get_sprite(id, "stock"), 0, _xx + o*25 - 40, _yy, 1, 1, 0, img_blend, 0.5)
						pal_swap_reset()
					}
				}
			}
			for (var o = 0; o < min(array_length_2d(obj_results.deaths, player_number), 4); o++) {
				...
				if (_inst >= 0) {
					with (other.player[_inst]) {
						pal_swap_set(pal_surface, 1, true)
						draw_sprite_ext(scr_get_sprite(id, "stock"), 0, _xx + o*25 - 40, _yy + 20, 1, 1, 0, img_blend, 0.5)
						pal_swap_reset()
					}
				}
			}
			draw_text_normal(_xx - 40, _yy - 35, "K")
			draw_text_normal(_xx - 40, _yy + 10, "D")
		} else {
			...
			var _spr = scr_get_sprite(id, "stock")
			draw_healthbar(_xx - 5, _yy, _xx + 40, _yy - 5, 100*shield_percentage/shield_max_percentage, c_black, c_red, c_green, 0, true, false)
			...
			pal_swap_set(pal_surface, 1, true)
			draw_sprite_ext(_spr, 0, _xx - 30, _yy, 3, 3, 0, image_blend, 1)
			pal_swap_reset()
		}
		if (spawning) {
			if (instance_exists(attacker)) {
				with (attacker) {
					pal_swap_set(pal_surface, 1, true)
					var _spr = scr_get_sprite(id, "stock")
					draw_sprite_ext(_spr, 0, _xx, display_get_gui_height(), 2, 2, 0, img_blend, 1 - (other.draw_count/GAME_SPEED))
					pal_swap_reset()
				}
			}	
		}
