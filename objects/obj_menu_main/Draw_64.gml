...
//set variables and draw parameters
var _yy = y
var _xx = x
...
for (var i = 0; i < array_length_1d(menu_option); i++) { //for each menu option
	...
	if (i = menu_index) { //if currently selected
		...
		if (menu_index = 4) { //if selecting exit
			//draw the menu option sprite darkened based on how close it is to exing
			...
			//draw mmenu option selected sprite
...
_yy = y
_xx = x
if (old_menu_index != menu_index) { //if changing menu indexes
	if (_percent > 0.5) { //if not yet half done
		//draw old menu sprite going off screen
	...
	} else { //more than half transitioned
