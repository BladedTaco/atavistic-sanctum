...
with (argument[2]) {//creator) {
	if ((inertial and (obj_match_handler.state[player_number] = HELPLESS) and !spawning and (point_distance(0, 0, momentum_x, momentum_y) > 2)) or (_override))  { //if colliding into ground
		...
		if (_l <= 3) {
			if (abs(angle_difference(point_direction(_inst.x, _inst.y, x, y), _inst.image_angle + 90)) < 90) {
				_d = 270	
			}
		}
		...
		for (var i = 0; i <= _i_max; i++) {
			...
			(_yy - _id._y[i])*(_id._x[(i+1) mod 4] - _id._x[i])) != _id.side[i]) { //outside line
				if (_d2 = -1) { //angle not set yet, so just get angle of line to bounce off
					_d2 = (point_direction(_id._x[i], _id._y[i], _id._x[(i+1) mod 4], _id._y[(i+1) mod 4]) + 90) mod 360
				} else { //get the average angle of the walls, which should end up in a straight reverse bounce
					_d2 = (_d2 + ((point_direction(_id._x[i], _id._y[i], _id._x[(i+1) mod 4], _id._y[(i+1) mod 4]) + 90) mod 360))/2
				}
			}
		}
		if (_d2 != -1) { //if a wall if found to bounce off of, bounce off it
			if !(_override) { //dont change sprite or delay momentum on override
				if (abs(angle_difference(_inst.image_angle, _d)) mod 170 < 10) {
					exit	
				}
