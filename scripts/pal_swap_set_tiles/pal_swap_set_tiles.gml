...
if(_low<_high) {   //You passed the arguments in backwards.
    _low=argument[3]-1;
    _high=argument[2]+1;
}    
...
with(obj_tile_swapper_start) {
    if(depth==_low) {   //Object already exists, update the values.
        _start_exists=true;
        active=true;
...
if(!_start_exists) {   //Object does not exist.  Create it.
...
with(obj_tile_swapper_end) {
    if(depth==_high) {   //Object already exists, update the values.
        _end_exists=true;
        active=true;
    }
}

if(!_end_exists) {   //Object does not exist.  Create it.
