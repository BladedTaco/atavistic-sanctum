...
if (!obj_input.sticky_dodge[argument[1]]) { //if able to dodge
	if (argument[2]) { //airborne therefore air dodge
		if (argument[0].shield_percentage > 0) { //if able to jump/air-dodge
			argument[0].shield_percentage -= 11 //decrement shield
			...
			scr_apply_impulse(argument[0], argument[1], argument[3], round(_IMPULSE._AIR_DODGE)/100, false) //apply airdodge impulse
		...
		if (argument[0].shield_percentage > 0) { //if able to jump/air-dodge
			argument[0].shield_percentage -= 7 //decrement shield
			if ((abs(round(argument[3] / 90) - 1) mod 2) = 1) { //direction is left or right
				...
				scr_apply_impulse(argument[0], argument[1], round(argument[3]/180)*180, round(_IMPULSE._ROLL)/100, false) //apply roll impulse
			} else {
