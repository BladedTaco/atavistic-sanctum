///@func scr_cir_col_rec(_id, _id2)
///@param _id - the id of the ellipse hitbox
///@param _id2 - the id of the rectangle hitbox
///@desc returns if a collision between the given rectangle and ellipse exists
...
if (scr_point_in_rec(_id.x, _id.y, _id2)) { return true } //return true


//check if the rotated bounding boxes are colliding else return 0
if (!scr_rec_col_rec(_id, _id2)) { return false} //return no collision if not true
...
for (var i = 0; i <= 3; i++) { 
	_xx = _id2._x[i]
	_yy = _id2._y[i]
	if (scr_point_in_cir(_xx, _yy, _id)) { return true }
}
...
for (var i = 0; i <= 3; i++) {
	...
	if (scr_point_in_rec(_xx, _yy, _id2)) { return true }
}
